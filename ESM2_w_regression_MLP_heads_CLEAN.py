...
class ProtDataModule(pl.LightningDataModule):
    ...
    def __init__(self, data_frame, regression_label_indices, ordinal_label_indices, batch_size, splits_path=None, splits_type=None, token_format='ESM2', seed=0):
    ...
    def mutations_split(self):
        ...
        for _, group in grouped:
            ...
            train, temp = train_test_split(indices, test_size=0.2, random_state=self.seed, shuffle=True)
            val, test = train_test_split(temp, test_size=0.5, random_state=self.seed, shuffle=True)
...
class finetuning_ESM2_with_mse_loss(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        elif self.embedding_type == 'max_pooling':
            # Use max of all tokens for embedding
            embedding, _ = torch.max(last_hidden_states, dim=1)  # [batch_size, hidden_size]
